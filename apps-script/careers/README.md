...
지원 데이터는 기존 열 순서를 유지하며, 마지막 열에 `합류 여부 안내` 값(`희망` 또는
`희망하지 않음`)을 추가합니다. 시트의 첫 행에도 동일한 이름의 헤더를 추가해 주세요.
운영 Apps Script가 갱신되기 전에도 값이 유실되지 않도록 프론트엔드는 자기소개 끝에
동일한 값을 함께 전송하며, 갱신된 스크립트는 이 호환 문구를 제거한 뒤 별도 열에 저장합니다.
