...
| 컴포넌트    | 기준                                                                                                                                                                                                                                                              |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Header      | `fixed` floating container와 `<Content>`의 container 폭 원칙을 따른다. 높이와 상단 보정은 기존 `--height-header` / `--padding-header` token을 단일 기준으로 사용하고, container radius는 `full`을 사용한다.                                                       |
| Header CTA  | `primary` 배경, `primary-foreground` 텍스트, `full` radius와 Label typography를 사용한다. Label button 규격에 따라 height는 48px, horizontal padding은 24px을 사용한다. 모바일에서는 숨기고 `md` 이상에서만 노출한다.                                             |
...
- Header navigation은 Body 2를 기본으로 사용하고, 데스크탑에서 시각적 위계가 더 필요하면 Body 1로 한 단계 올린다. Label과 크기가 같아도 navigation과 control의 semantic role을 구분한다.
...
- 현재 상시 header shadow와 `/work`의 `shadow-card`는 canonical rule이 아니라 `Legacy / Migration Notes`의 정리 대상이다.
...
| ID   | 섹션          | 핵심                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---- | ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S-01 | 헤더(GNB)     | sticky 고정, 스크롤 시 배경색 유지+옅은 그림자만. 로고→최상단 스무스 스크롤. 앵커 5개(똑독은→S-03 / 우리 동네에서→S-04 / 보호자와 함께→S-05 / 원장님과 함께→S-06 / 무엇이든→S-07), 현재 섹션 항목 강조. 헤더 높이만큼 스크롤 오프셋. 우측 "앱 다운로드" CTA는 `md` 이상에서만 노출하고 모바일 헤더에서는 숨긴다.                                                                                                                                                                                                                                                                                               |
| S-02 | 히어로        | 서브카피 1줄 + 대형 헤드라인("탐색하고," / 이미지 위 오버레이 "연결하고, 기록하는.") + 보호자·반려견 실사 이미지 1장(PC/모바일 분리) + 하단 `scroll` bounce(md 이상만 노출). 모바일 headline은 `text-display-3`, supporting copy는 `text-body-1`을 사용한다. 모바일 앱 다운로드 버튼은 `text-label`과 Label button 규격을 사용한다. 모바일 hero content는 header 아래부터 hero 하단까지의 영역 안에서 좌하단 정렬(hero 하단에서 48px 여백)하고, headline→supporting copy 16px / supporting copy→CTA 20px 간격을 사용한다. 모바일 hero image object-position은 `52% 30%`를 사용한다. 좌우 스플릿 정적 레이아웃. |
...
| `/work`의 card                 | 기본 상태의 `shadow-card`              | background / border / spacing 중심으로 구분하고 기본 shadow 제거 |
...
| Header outer layout            | 별도 container / padding               | 다음 Header 작업에서 공통 Content 기준선에 정렬                  |
